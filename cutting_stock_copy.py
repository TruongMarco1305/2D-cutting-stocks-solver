...
class CuttingStockEnv(gym.Env):
    ...
    def __init__(
        ...
        # Set seed explicitly
        set_seed(seed)
        # self.total_used_area = 0  # CHANGED
        # self.total_area = 0        # CHANGED
    ...
    def _get_info(self):
        ...
        for sid, stock in enumerate(self._stocks):
            if self.cutted_stocks[sid] == 0:
                continue
            stock_area_used += (stock != -2).sum()
            tl = (stock == -1).sum() / (stock != -2).sum()
            trim_loss.append(tl)

        trim_loss = np.mean(trim_loss).item() if trim_loss else 1
        # Tính tổng diện tích sử dụng (đơn vị: số ô đã được cắt)

        # filled_ratio = (self.total_used_area / self.total_area) if self.total_area > 0 else 0
        ...
        return {"filled_ratio": filled_ratio, "trim_loss": trim_loss, "stock_area_used": stock_area_used}
    ...
    def reset(self, seed=42, options=None):
        ...
        self._stocks = []
# Randomize stocks
        stock_w = [40, 55, 70, 80, 95, 60, 78, 50, 70, 90]
        stock_h = [50, 45, 60, 65, 75, 30, 48, 90, 70, 90]
        stock_q = [10, 14, 4, 17, 11, 7, 12, 11, 8, 6]
        j = 0
        num_type_stocks = 10
        for width, height, quantity in zip(stock_w, stock_h, stock_q):
            stock = {"size": np.array([width, height]), "quantity": quantity}
        ...
        prod_w = [23, 28, 43, 33, 63, 14, 20, 32]
        prod_h = [18, 23, 38, 28, 53, 34, 22, 59]
        prod_q = [16,12,12, 14, 11, 15, 5, 10]
        i = 0
        num_type_products = 8
        ...
        #     self._stocks.append(stock)
        # # self.total_area += (stock != -2).sum()  # CHANGED
        # self._stocks = tuple(self._stocks)
            ...
            if 0 <= stock_idx < self.num_stocks:
                ...
                if (
                    ...
                    if np.all(stock[x : x + width, y : y + height] == -1):
                        ...
                        # self.total_used_area += width * height  # CHANGED
