...
class CuttingStockEnv(gym.Env):
    ...
    def _get_info(self):
        filled_ratio_martinDuc = np.mean(self.cutted_stocks).item()
        trim_loss = []
        stock_area_used = 0
        total_area = 0
        ...
        return {"area_filled_ratio": stock_area_used / total_area, "filled_ratio": filled_ratio_martinDuc, "trim_loss": trim_loss, "stock_area_used": stock_area_used, "total_area": total_area}
    ...
    def reset(self, seed=42, options=None):
        ...
        # # Randomize stocks
        # stock_w = [40, 55, 70, 80, 95]
        # stock_h = [50, 45, 60, 65, 75]
        # stock_q = [20,20,20,20,20]
        # j = 0
        # for _ in range(len(stock_w)):
        #     # width = np.random.randint(low=self.min_w, high=self.max_w + 1)
        #     for _ in range(stock_q[j]):
        #         width = stock_w[j]
        #         height = stock_h[j]
        #         # height = np.random.randint(low=self.min_h, high=self.max_h + 1)
        #         stock = np.full(shape=(self.max_w, self.max_h), fill_value=-2, dtype=int)
        ...
        # self._products = []
        # # num_type_products = np.random.randint(low=1, high=self.max_product_type)
        # prod_w = [20, 25, 40, 50, 60, 85]
        # prod_h = [15, 20, 35, 45, 50, 65]
        # prod_q = [10, 8, 22, 4, 3, 4]
        # # prod_w = [39, 18, 11]
        # # prod_h = [14, 2, 14]
        # # prod_q = [52,51,50]
        # i = 0
        # num_type_products = len(prod_w)
        # for _ in range(num_type_products):
        #     # width = np.random.randint(low=1, high=self.min_w + 1)
        #     # height = np.random.randint(low=1, high=self.min_h + 1)
        #     width = prod_w[i]
        ...
        self._products = []
        stock_w = [53, 52, 55, 68, 73]
        stock_h = [56, 59, 81, 78, 62]
        stock_q = [20,20,20,20,20]
        j = 0
        for _ in range(len(stock_w)):
            # width = np.random.randint(low=self.min_w, high=self.max_w + 1)
